...
impl ConfigPersister {
    ...
    fn generate_game_ini_content(config: &ServerConfig) -> String {
        ...
        for i in 0..10 {
            let value = match i {
                0 => config.dino_wild_stats.health,
                1 => config.dino_wild_stats.stamina,
                2 => config.dino_wild_stats.oxygen,
                3 => config.dino_wild_stats.food,
                4 => config.dino_wild_stats.water,
                5 => config.dino_wild_stats.weight,
                6 => config.dino_wild_stats.melee_damage,
                7 => config.dino_wild_stats.speed,
                8 => config.dino_wild_stats.fortitude,
                9 => config.dino_wild_stats.torpidity,
                _ => 1.0,
            };
            content.push_str(&format!("PerLevelStatsMultiplier_DinoWild[{}]={}\n", i, value));
        }
    ...
    fn generate_gamesettings_ini_content(config: &ServerConfig) -> String {
        ...
        content.push_str(&format!("MaxTamedDinos={}\n", config.advanced.max_tamed_dinos));
